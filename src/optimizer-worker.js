...
async function verifyWasmForEffect(effect){
  ...
  try{
    // wasmBatchScores shares duration/trials/seed/base parameters from the first
    // candidate in a batch. Verify each deliberately varied case separately so
    // the validator never tests an unsupported mixed-configuration batch.
    for(const c of cases){
      const got=wasmBatchScores([c]);
      const ref=runSimulation(c);
      if(!got||got.length!==1||!sameScore(got[0],ref))return false;
    }
    return true;
...
async function scoreMany(cfgs){
  ...
  if(work.length && await ensureWasm()){
    ...
    if(wasmSafe[effect] && wasmCompatible(work,effect)){try{const r=wasmBatchScores(work);if(r){for(let k=0;k<misses.length;k++){out[misses[k].i]=r[k];scoreCache.set(misses[k].key,r[k]);}return out;}}catch(e){wasmSafe[effect]=false;}}
  }
...
self.onmessage=async function(ev){const d=ev.data||{};try{
 ...
 if(d.cmd==='screen'){const eq=d.equipment,base=shared.base,rots=shared.compiledRotations||shared.rotations||[];const cfgs=rots.map(rot=>({...base,equipment:eq,rotation:rot,policy:null,duration:d.duration,trials:d.trials,seed:d.seed}));const rs=await scoreMany(cfgs);let best=null,row=[];for(let i=0;i<rs.length;i++){const x={rotation:rots[i],score:rs[i].uptime};row.push(x);if(!best||x.score>best.score)best=x;}self.postMessage({cmd:'result',id:d.id,result:{best,row}});return;}
 if(d.cmd==='policyScreen'){const eq=d.equipment,base=shared.base,ps=shared.policies||[],fallback=cachedRotation(d.fallback);const cfgs=ps.map((p,i)=>({...base,equipment:eq,rotation:fallback,policy:(shared.compiledPolicies&&shared.compiledPolicies[i])||policyFromSpec(p),duration:d.duration,trials:d.trials,seed:d.seed}));const rs=await scoreMany(cfgs);const out=rs.map((r,i)=>({index:i,score:r.uptime}));out.sort((a,b)=>b.score-a.score);self.postMessage({cmd:'result',id:d.id,result:{top:out.slice(0,64)}});return;}
 if(d.cmd==='policyExpand'){const eq=d.equipment,base=shared.base,rots=shared.compiledRotations||shared.rotations||[],ps=shared.policies||[],leaders=d.leaders||[];const jobs=[];for(const leader of leaders){const p=ps[leader.index];for(const rot of rots)jobs.push({leader,rot,c:{...base,equipment:eq,rotation:rot,policy:policyFromSpec(p),duration:d.duration,trials:d.trials,seed:d.seed}});}const rs=await scoreMany(jobs.map(x=>x.c));const bests=[];for(let i=0;i<leaders.length;i++){let best=null;for(let j=0;j<rots.length;j++){const z=jobs[i*rots.length+j],sc=rs[i*rots.length+j].uptime;if(!best||sc>best.score)best={index:z.leader.index,score:sc,rotation:z.rot};}bests.push(best);}self.postMessage({cmd:'result',id:d.id,result:{bests}});return;}
 if(d.cmd==='batchRun'){const list=d.candidates||[];const rs=await scoreMany(list);self.postMessage({cmd:'batchResult',id:d.id,results:rs});return;}
 if(d.cmd==='run'){const rs=await scoreMany([d.cfg]);self.postMessage({cmd:'result',id:d.id,result:rs[0]});return;}
